...
class mainClass {
...

	{
        ...
        $sql="SELECT DISTINCT ct_alias, ct_titulo, ct_id, ct_orden,ct_orden_home FROM `tbl_categorias` 

		INNER JOIN tbl_productos ON tbl_productos.pd_categoria=tbl_categorias.ct_mla 

		WHERE ct_id!=0 AND status='publicado' AND ct_mostrar_home='si' 
        ORDER BY ct_orden_home ASC";
...

	{
		...
		if($action == 'ajax'){
            ...
            if(!empty($this->pd_etiqueta)){

                if(!strstr($sql,"WHERE")){

                    $sql.=" WHERE pd_etiqueta='".$this->pd_etiqueta."'";

                }else{

                    $sql.=" AND pd_etiqueta='".$this->pd_etiqueta."'";
	...
	/*public function productosFeed_old()
...

	{
        ...
        if ($query->rowCount()>0) {
            ...
            {   
                ...
                $queryImg = $this->conn->prepare("SELECT im_400x400, im_800x800, im_padding, IF(im_externa='si', -1, im_orden) AS orden FROM `tbl_img` WHERE im_producto=? ORDER BY orden ASC");
                ...
                if ($queryImg->rowCount()>0) {
                    ...
                    while($img = $queryImg->fetch()){

                        $reg_img['imagen']=$img['im_800x800'];

                        $reg_img['padding']=$img['im_padding'];

                        $reg['imagenes'][] = $reg_img;
                    }


                } else {

                    $reg_img['imagen'] = 'sin-imagen.jpg';
                    $reg['imagenes'][0] = $reg_img;
